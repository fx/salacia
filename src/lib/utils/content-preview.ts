...
const PREVIEW_CONFIG = {
  ...
  /** Patterns to skip in user message preview generation */
  SKIP_PATTERNS: {
    /** Tool result prefixes */
    TOOL_RESULT_PREFIXES: ['🔧 Tool Result', '[Tool Result]'],
    /** System content indicators */
    SYSTEM_INDICATORS: ['<system-reminder>', 'claudeMd', 'Codebase and user instructions'],
  },
} as const;
...
function shouldSkipUserMessage(content: string): boolean {
  // Check if content starts with any tool result prefix
  const hasToolResultPrefix = PREVIEW_CONFIG.SKIP_PATTERNS.TOOL_RESULT_PREFIXES.some(prefix =>
    content.startsWith(prefix)
  );

  // Check if content includes any system indicator
  const hasSystemIndicator = PREVIEW_CONFIG.SKIP_PATTERNS.SYSTEM_INDICATORS.some(indicator =>
    content.includes(indicator)
  );

  // Check if content exceeds maximum length
  const isTooLong = content.length >= PREVIEW_CONFIG.MAX_USER_MESSAGE_LENGTH;

  return hasToolResultPrefix || hasSystemIndicator || isTooLong;
}
